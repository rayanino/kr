...
<stakes>
The passaging engine creates the PROCESSING UNITS that every downstream engine operates on. A bad passage boundary splits arguments, breaks context, and produces incoherent excerpts that mislead the owner. The library IS the owner's knowledge — an error in the pipeline is an error in his mind. Read KNOWLEDGE_INTEGRITY.md for the 7 corruption threats.
</stakes>
