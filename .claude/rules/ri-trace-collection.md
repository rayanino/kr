...
- When reviewing ri improvement proposals (`eval/action_plan.md` (generated by `recursive-improve eval`; does not exist until first trace analysis run)), run `python scripts/ri_review.py` to auto-reject content-affecting changes before manual review.
