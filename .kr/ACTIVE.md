...
Status: active — Phase 5b items 1, 2, 3, 12, 13, 14, 15, 16 closed (commits `62647cb2b`, `ec8d82ca4`, `386685819`, `bf4354399`+`a4f2b2788` retroactive-review amendments, `f965aec7b`, `a1fabdd54`+`366463bcb` item-16); **Item 4 BLOCKED 2026-04-22 by scholarly category error (all 4 options rejected by 2-run Gemini unanimous BLOCKER_PRESENT); Option E drafted, awaiting fresh dispatch cycle.** **7 Phase 5b work items remain (5, 6, 7, 8, 9, 10, 11)** plus 3 follow-ups from 12-14 retroactive (17, 18 remain; 16 DONE) plus 2 from item-16 dispatch (19, 20) plus 3 from item-4 dispatch (21, 22, 23)
...
4. Amend REQ-SRC-0030 genre enum to cover non-applicable set; reconcile non-applicable list across CON-SRC-0004/REQ-SRC-0047/INV-SRC-0012 to a single canonical source; sub-classify hadith_collection (remove unconditionally-non-applicable placement). **STATUS 2026-04-22: Pre-commit 3-evaluator dispatch (Codex CLI structural + Gemini CLI 2 independent scholarly runs, all through `/prompt-architect`) evaluated 4 drafted options (A/B/C/D). Codex PREFER_A (bounded); BOTH Gemini runs BLOCKER_PRESENT unanimously on DIM1 classical-taxonomy category errors:** (a) `majmu` is a composite structure (already covered by SourceMetadata.composite_work_type), not a fann — elevating it to top-level Genre commits T-3 Taxonomic Misplacement; (b) `biographical_dictionary` is the English alias for `tarajim`, which is the parent category of existing Genre.TABAQAT — adding as sibling flattens taxonomy; (c) `rijal_dictionary` is a nawʿ of tarikh/tabaqat/hadith sciences (exemplar: Ibn Abī Ḥātim's al-Jarḥ wa-l-Taʿdīl), not an independent fann. Run A and Run B also AMEND on DIM2: KEEP `Genre.FATAWA` and `Genre.MUJAM` — `Fatāwā` is the canonical classical name (Fatāwā al-Subkī, Fatāwā Ibn Taymiyya) and `muʿjam` has a distinct hadith-science usage (organized by shuyūkh, e.g., al-Muʿjam al-Kabīr of al-Ṭabarānī) that "lexicon" erases. Per `.claude/rules/no-single-model-conclusion.md`, unanimous scholarly BLOCKER outranks structural PREFER_A. **No commit made.** **Option E (drafted, awaiting own dispatch cycle):** (1) Add only `MUSHAF = "mushaf"` to Genre enum — it is the only genuinely-new top-level fann among the 6 candidates; (2) DO NOT add `majmu`, `rijal_dictionary`, or `biographical_dictionary` to Genre — they are not fann-level; (3) DO NOT rename FATAWA/MUJAM — preserve classical canonical forms; (4) Extend the non-applicability gate in deliberation.py to check Genre OR SourceMetadata.composite_work_type (for majmu-type composites) OR HadithSubgenre (for hadith sub-classification deferred to item 23); (5) Extend INV-SRC-0012 rule statement to reflect the gate-redesign (3 orthogonal axes), not a flat 7-value frozenset. Option E requires fresh 3-evaluator dispatch before commit. Reviewer outputs at `.kr/runtime/structural_audit_codex_cli_item4_precommit_20260421.md`, `.kr/runtime/domain_validation_gemini_cli_item4_run_A_20260421.md`, `.kr/runtime/domain_validation_gemini_cli_item4_run_B_20260421.md` (gitignored). Dispatch log entries appended to `.kr/runtime/dispatch_log.jsonl`.
