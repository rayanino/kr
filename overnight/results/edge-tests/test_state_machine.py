...
class TestImpossibleStates:
    ...
    def test_09_page_range_start_greater_than_end(self) -> None:
        """PageRange with start_page > end_page should be constructible.

        The pipeline does not currently validate this at the Pydantic level.
        This test documents the gap: an inverted range can be created
        without error. This is an edge case for downstream consumers.
        """
        # This should NOT raise — PageRange has no start<=end validator
        pr = PageRange(volume=1, start_page=50, end_page=10)
        assert pr.start_page > pr.end_page, (
            "PageRange allows inverted ranges (no validator) — "
