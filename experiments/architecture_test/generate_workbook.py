...
def generate_workbook(divisions: list[dict]) -> str:
    ...
    sections.append("# Architecture C — Evaluation Workbook\n")
    sections.append(
    ...
    for div in divisions:
        ...
        result_c = load_result(fixture, div_start, "c")
        if result_c:
            n_c = result_c.get("total_units", len(result_c.get("teaching_units", [])))
            sections.append(f"### Approach C Results ({n_c} teaching units)\n")
            sections.append(format_teaching_units_table(result_c))
            sections.append("")
        ...
        if result_c:
            n_c = result_c.get("total_units", 0)
            if result_b:
                n_b = result_b.get("total_units", 0)
                if n_b != n_c:
                    notes.append(f"- C (with context) produced {n_c} units vs B's {n_b} — context changed grouping")
                else:
                    notes.append(f"- C produced same unit count as B ({n_c}) — context did not change grouping")
