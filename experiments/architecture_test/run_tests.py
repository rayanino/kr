"""Deliverable 3: Run LLM teaching-unit extraction tests on all 10 divisions.

Three approaches:
  A — Single-call joint extraction
  B — Two-call classify-then-group
  C — B with cross-boundary context (03_fiqh + mid_argument divisions only)

All calls go through OpenRouter using anthropic/claude-opus-4.6.
Results saved to experiments/architecture_test/results/{fixture}/{div}_{approach}.json.
"""
...
APPROACH_C_GROUP_SYSTEM = """You are an expert in classical Islamic scholarly text analysis.

You previously classified segments of the MAIN TEXT by scholarly function.
Now group these classified segments into TEACHING UNITS — self-contained scholarly
segments that each teach one distinct concept, ruling, or argument.
...
def run_approach_c(
    ...
    """Approach C: Classify-then-group with cross-boundary context."""
    # Call 1: Classify segments (same as B)
    classification, _ = call_llm(
        client=client,
        system_prompt=APPROACH_B_CLASSIFY_SYSTEM,
        user_message=division["assembled_text"],
    ...
    # Call 2: Group with cross-boundary context
    context_before = division.get("context_before", "")
    context_after = division.get("context_after", "")

    user_msg = (
        "[PREVIOUS DIVISION — for context only, do not extract units from this]\n"
        + (context_before if context_before else "(no previous context)")
        + "\n\n[MAIN TEXT — extract teaching units from this only]\n"
        + division["assembled_text"]
        + "\n\n[NEXT DIVISION — for context only, do not extract units from this]\n"
        + (context_after if context_after else "(no next context)")
        + "\n\nIf any argument at the start or end of the MAIN TEXT is incomplete, note this\n"
        + "in the teaching unit's self_containment_notes and mark self_contained=False.\n\n"
    ...
    extraction, _ = call_llm(
        client=client,
        system_prompt=APPROACH_C_GROUP_SYSTEM,
        user_message=user_msg,
        response_model=ExtractionResult,
        fixture=division["fixture_name"],
        div_id=f"div_{division['div_start_unit']}",
...
# ──────────────────────────────────────────────────────────────────
# Division Loading + Approach C Decision
# ──────────────────────────────────────────────────────────────────
...
def should_run_c(division: dict) -> bool:
    """Determine if Approach C should be run on this division."""
    # All 03_fiqh divisions
    if division["fixture_name"] == "03_fiqh":
        return True
    # Any division with mid_argument BC on last unit
    bc_last = division.get("boundary_continuity_last_unit")
    if bc_last and bc_last.get("type") == "mid_argument":
        return True
    return False
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        for div in divisions:
            ...
            f.write(
                ...
                f"| {a_units} | {b_units} | {c_units} | {error_str} |\n"
            )
...
def main() -> None:
    ...
    for div in divisions:
        ...
        try:
            ...
            # Also save the intermediate classification
        ...
        if should_run_c(div):
            try:
                result_c, classification_c = run_approach_c(client, div)
                save_result(fixture, div_start, "c", result_c)
                save_result(fixture, div_start, "c_classify", classification_c)
                tracker["c_units"] = result_c.total_units
                print(f"  C: {result_c.total_units} teaching units ({classification_c.total_segments} segments)")
            except Exception as e:
                save_error(fixture, div_start, "c", e)
                tracker["c_units"] = "ERR"
                tracker["errors"].append(f"C:{type(e).__name__}")
                print(f"  C: ERROR - {e}")
        else:
            tracker["c_units"] = "—"
            print("  C: skipped (not applicable)")
    ...
    print("=" * 100)
    print(f"{'Fixture':<20} {'Div':>5} {'Heading':<42} {'Words':>6} {'A':>4} {'B':>4} {'C':>4} {'Errors'}")
    print("-" * 110)
    ...
    for div in divisions:
        ...
        print(
            ...
            f"{r.get('b_units', '?'):>4} {r.get('c_units', '?'):>4} {error_str}"
        )
