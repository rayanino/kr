...
- `_pending_` docs(source): close OQ-SRC-0005 in active frontier
...
**Open follow-ups:** 18 (FU-27, FU-28, FU-36 closed at preceding commits in this session). **Deferred SPEC atoms: {} — last deferred atom OQ-SRC-0005 superseded at commit `49a864ffe`; first time the source-engine spec has zero deferred atoms since freeze 2026-04-15.**
...
1. **Close FU-18** — only remaining open follow-up: taxonomy expansion for scholarly-nuance dispute-tier severity beyond fatal/blocking/warning. Substantive — requires a fresh 4-evaluator dispatch wave (similar scope to FU-36) per `.claude/rules/no-single-model-conclusion.md`. Apply the FU-37 sealed-block-in-separate-file rectification (place sealed prior-evaluator block at a path Geminis can Read via tool call — NOT under `.kr/` which is gitignored; suggested location `engines/source/spec/60-evidence/_sealed_fu18_*.md` or fresh `engines/source/_dispatch/` path). Recommend fresh session per `.claude/rules/context-management.md` "one engine per session" discipline.
