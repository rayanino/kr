...
**Recent build activity (2026-05-09):**
- `8de6130f6` feat(normalization): Phase 5 Session 12 — P3 broader determinism audit sweep across the 4 in-scope engines (normalization, atomization, passaging, synthesis) not yet covered by Sessions 7+10+11. **Validation gates ALL GREEN:** pyright 0/0 on the touched src file (after adjacent enumerate cleanup at lines 304+364 dropping pre-existing reportUnusedVariable warnings); pytest engines/normalization/tests/ **513 pass / 14 skip / 0 fail** (+7 from baseline 506p/14s/0f); pytest engines/source/tests/ + shared/scholar_authority/tests/ **749 pass / 1 skip / 0 fail** (Session 11 baseline preserved exactly); verify_metadata_flow 5 baseline UNPLACED warnings unchanged; validate_spec 124a/0e/118 confirmed/6 superseded unchanged. **Survey results across the 4 in-scope engines (30 sites total):** engines/normalization/src/ surveyed 29 sites (28 SAFE + 1 RISK); engines/atomization/src/ has 0 sort/sorted/max/min sites in src/ (only docstring mentions of "sorted"; verified by exhaustive grep — `validator.py` V-4 docstring mentions "atoms sorted by anchor_span.start" describing behavior, not source code); engines/passaging/src/ has 0 sort sites in src/ at all (verified by exhaustive grep; the engine's deterministic ordering is provided by upstream contracts.py invariants); engines/synthesis/src/ has 1 site at `tracer.py:109` `staleness_input = json.dumps(sorted(e["excerpt_id"] for e in excerpts))` — sorted str excerpt_ids for staleness hash, classified SAFE per matrix row 2 (sorted set/list of total-order primitives with no expected duplicates). **Single RISK at engines/normalization/src/structure_discovery.py:1128** (`_build_page_markers`): `min(page_cands, key=lambda c: c.document_position)` was single-keyed over a sequence where `_detect_volume_boundaries` (line 750-757) omits `document_position` when constructing HeadingCandidate, defaulting to 0 per the dataclass field default at line 86; collides with Tier 1's first candidate (also doc_pos=0) on pages where both a chapter heading and a volume boundary land on the same unit_index. Tie-resolution previously relied on list-append ordering — fragile-by-coincidence determinism that the audit rule exists to catch (Session 9 §4 had not deep-investigated this site since it was MEDIUM-classified pending the rule promotion in Session 11's c9512e2dd commit). **The fix:** tuple-key `(c.document_position, c.heading_text)` per Sessions 5+7+10 pattern. `heading_text` is the required str field at line 78 (no default, no Optional) so it satisfies the audit rule's "total-order + always-populated" requirement. Strict refinement: when document_position values are unique, the prior winner still wins; only ties consult heading_text lexicographically. **Adjacent fix per "Fix adjacent broken code" override:** dropped 2 unused enumerate() loop variables at lines 304 (span_idx in _tier1_html_tagged) and 364 (line_i in _tier1_5_toc_parse), clearing pre-existing pyright reportUnusedVariable warnings unrelated to the determinism fix but blocking the quality gate hook. **NEW** engines/normalization/tests/test_phase5_session12_determinism.py ~190L 7 tests locks the contract per audit rule's test pattern: (a) two distinct input orderings of identical items produce identical output, (b) repeated calls (10x) produce identical output (catches PYTHONHASHSEED non-determinism), (c) Arabic byte-faithful through the tiebreak (no normalization, no diacritic stripping), (d) primary-key strict-ordering preserved when no ties, (e) three-way permutation tie, (f) cross-page full-marker-dict equality, (g) singleton-page no-op. Real Arabic fixtures: classical fiqh chapter headings (كتاب الطهارة, كتاب الصلاة, باب الوضوء, فصل في النية, المجلد 2, مقدمة, باب التيمم) per `.claude/rules/testing.md`. Diacritics-heavy fixture (كِتَابُ الطَّهَارَةِ) verifies T-1 byte-faithfulness through the comparison path. **Defect class generalization:** Sessions 5+7+10+12 = **4-of-4 confirmed PYTHONHASHSEED tiebreak defect class** (Session 5/7's `_build_positions_for_disputed`; Session 10's `route_excerpt` + `place_excerpt` + `compute_layer_attribution`; Session 12's `_build_page_markers`). The `(-primary, secondary_total_order)` tuple-key pattern from `.claude/rules/single-key-sort-audit.md` (committed Session 11 at c9512e2dd) continues to hold as the durable form across all 4 cases. **Codex CLI structural review:** CONFIRM_HIGH at HIGH confidence on round 1. 8/8 verification questions PASS with file:line evidence; defects=[]; new_defects_found=0; explicit_non_defects_count=6 (target ≥5; named: required_string_secondary, volume_heading_wins_equal_doc_pos, primary_key_still_dominates, three_way_arabic_tie, diacritics_in_comparison_path, singleton_pages_after_tuple_key); recommended_action=COMMIT; ac_verdicts_from_body_inspection=8; ac_verdicts_from_name_only=0; test_bodies_read_count=7 (all 7 tests, exceeds Session 11's 5); files_reviewed=5; tokens_used=45,004. T-1 silent_corruption PASS; T-2 attribution_error PASS; T-6 metadata_poisoning PASS. **Withhold-one-detail strategy worked:** the secondary-key field name (`heading_text`) was intentionally NOT inlined in the reviewer prompt — Codex correctly identified it by reading line 1132 directly, with explicit citation `engines/normalization/src/structure_discovery.py:78,82-86,1132` showing it independently traced the dataclass field declarations to verify total-order + always-populated requirements. Sessions 5+6+7+9+10+11+12 = **7-of-7 successful CONFIRM_HIGH/HIGH/COMMIT round-1 close** in the Phase 5 series. **Second consecutive real Codex CLI round-1 CONFIRM_HIGH** (Sessions 11 + 12; Codex CLI quota stable post 2026-05-08T10:33Z reset). Phase 5 series total on remote post-Session 12 feat push: **30 commits** + Session 12 docs (`_pending_session12_docs_`). **Active sub-frontier post-Session-12:** (1) **Priority 4 D2 progressive enrichment seeding** — `library/registries/scholars.json` is currently `{}`; AC-1 (Session 9) + AC-2/AC-3 (Session 11) admission paths are now ready to populate it organically through real-source ingestion; multi-session strategic (Session 13+); requires real source fixtures + intake-analysis pipeline runs; (2) **deeper Priority 3 sweep** — the 47 remaining MEDIUM/SAFE sort/sorted call sites surveyed in Session 9 §4 across engines/source/, engines/excerpting/, engines/taxonomy/, engines/atomization/, engines/passaging/, engines/synthesis/, shared/scholar_authority/ are out of P3 deterministic-test scope (Sessions 7+10+11+12 covered the HIGH-risk and 4-engine-survey scope); a deeper deep-investigation across the remaining MEDIUM sites could surface more defect-class instances if any remain, ~1-2h, can parallelize; (3) **future enrichment via Priority 4** — once D2 seeding produces a real scholar registry, real-source ingestion will exercise AC-1 + AC-2 + AC-3 paths against actual content, producing the first real verifier_dispatch persistence runs.
