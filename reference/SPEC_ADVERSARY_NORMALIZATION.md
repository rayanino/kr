...
**SPEC lines:** 2,047
**Total adversarial cases:** 42
**By category:** boundary_value: 10, arabic_trap: 10, silent_corruption: 9, format_edge_case: 8, multi_signal_conflict: 5
...
### ADV-022 arabic_trap — Python str.strip() removing diacritics at string edges

**SPEC rule:** "Preserve all diacritics exactly."
**Adversarial input:**
```python
text = "بِسْمِ"  # Ends with kasra (U+0650) as the last codepoint
# str.strip() on this text would NOT strip diacritics (they're not whitespace)
# BUT: text = "بِسْمِ\u200B" — ZWS at end, then:
# text.strip() would strip the ZWS, which is correct per SPEC (ZWS → PRESERVED)
# HOWEVER: text = "بِسْمِ " — trailing space before a diacritic rearranged by normalization
```

Actually, the real trap:
```python
text = " بِسْمِ اللَّهِ "  # Leading and trailing spaces around diacritized text
# str.strip() is safe here — removes spaces, not diacritics
# BUT: custom strip that removes Arabic combining marks would be catastrophic
```

**Correct behavior:** Leading/trailing whitespace is trimmed per §4.A.8 ("Leading/trailing line whitespace trimmed"). Diacritics at string edges are PRESERVED.
**Wrong behavior (naive impl):** Uses a regex like `re.sub(r'[\u0600-\u06ff]+$', '', text)` that catches combining marks at edges. Or uses `unicodedata.normalize()` before stripping.
**Why it matters:** Diacritics at text boundaries are valid (e.g., kasra on the last letter of a basmala). Stripping them is silent corruption.
**Detection:** Assert diacritics at first and last Arabic characters are identical between source and output.
...
**Why it matters:** Shamela exports sometimes mix digit forms. Missing Arabic-Indic markers means ~30% of footnote references could be orphaned.
...
**Correct behavior:** The SPEC says `>70%` (strictly greater than). At exactly 70.0%, the check fails — the page is flagged as potentially corrupted.
**Wrong behavior (naive impl):** Uses `>= 70%` → the page passes at exactly 70%. Or doesn't strip whitespace/punctuation from the denominator.
**Why it matters:** Off-by-one at a corruption detection boundary. A page at exactly 70% could be legitimate mixed Arabic/Latin scholarly text — but the SPEC draws the line here.
**Detection:** Assert page at 70.0% is flagged. Assert page at 70.1% passes.
...
|--------|------------------|-----------|
| T-1 (Silent Text Corruption) | ADV-006, ADV-007, ADV-021, ADV-022, ADV-039 | Diacritics, ZWNJ, NFC, entity corruption, semantic confusion |
| T-2 (Attribution Error) | ADV-011, ADV-012, ADV-013, ADV-014, ADV-015 | Bold threshold, multi-signal, layer coverage, metadata override |
