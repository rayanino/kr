...
### D-042: Owner confidence calibration moves from human gate to user model
**Decided:** 2026-03-06
**Context:** The human gate SPEC §4.A.4 stores owner confidence levels per science in `library/gates/confidence.json`. The user model (D-017) tracks per-science expertise levels, which is architecturally the same data. Having two separate stores for user expertise creates a consistency risk and splits user state across components.
