...
## SPEC-NOTE-4: §4.3 mid_sentence word-final heuristic produces 92% word-merge corruption — OPEN

**Found:** Excerpting Session 1 review (March 2026).
**Severity:** T-1 (Silent Text Corruption). Blocks Phase 2 deployment.
**Problem:** SPEC §4.3 defines mid_sentence separator as empty string `""`, with a word-final heuristic (ة, ى, tanwin, whitespace) to insert a space when the page break falls between words. Empirically measured across all 5 experiment fixture packages: 270 of 294 (92%) mid_sentence boundaries merge two separate Arabic words into garbage (e.g., "للخطأوَلِهَذَا", "كالشاهدوفتواه"). The heuristic catches only 8% of actual word boundaries because most Arabic words end with regular consonants (ن, ل, ب, د, etc.). CC implemented the SPEC faithfully — this is a SPEC design flaw.
**Fix direction:** Invert the default. Insert space for mid_sentence unless the prev text ends with a connecting letter form (non-final glyph shape) or a diacritic other than tanwin. Alternatively, always insert space — the cost of an extra space in genuine mid-word splits is far lower than concatenating 92% of between-word boundaries.
**Status:** OPEN. Architect must design corrected heuristic before Phase 2 deployment.

## SPEC-NOTE-5: §4.3 tanwin fathah + alif (ـًا) not detected as word-final — OPEN

**Found:** Excerpting Session 1 review (March 2026).
**Severity:** Low. 1 occurrence in 294 mid_sentence boundaries across 5 fixture packages.
**Problem:** The accusative indefinite form "كتابًا" ends with alif (U+0627) as the last code point, with tanwin fathah (U+064B) as a combining character before it. The heuristic checks only the last code point, missing this form.
**Status:** Subsumable into SPEC-NOTE-4's broader fix. Not independently urgent.
