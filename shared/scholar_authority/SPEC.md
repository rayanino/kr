...
**Step 3: Full matching.**

> **Phase 5 amendment 2026-04-30 (scholar-matching DR cross-provider synthesis, OPT-4 architecture, 4-evaluator wave 3-of-4 cross-provider HIGH).** The single-threshold auto-match / ambiguous / no-match routing described below is **superseded** for the source-engine call path by the source-engine atoms `engines/source/spec/.../REQ-SRC-0049.yaml` (snapshot locking), `REQ-SRC-0050.yaml` (fragment normalization), `REQ-SRC-0051.yaml` (deterministic candidate generation with work-title channel), `REQ-SRC-0052.yaml` (verifier cell with hybrid round-0/round-1 protocol), `REQ-SRC-0053.yaml` (disagreement routing with compound 4-condition threshold), `INV-SRC-0013.yaml` (≥2-non-name floor), `INV-SRC-0014.yaml` (matching-key honorific exclusion + bidi-strip ordering), `INV-SRC-0015.yaml` (provenance completeness), `INV-SRC-0016.yaml` (verifier chosen_id closure F-4), `INV-SRC-0017.yaml` (registry snapshot version pin F-7), `CON-SRC-0008.yaml` (ScholarMatchResult contract with dual state surface), `CON-SRC-0009.yaml` (evidence packet contract). The source-engine call path uses the OPT-4 2-stage deterministic-then-LLM-consensus architecture; results are emitted as `CON-SRC-0008` ScholarMatchResult with `disambiguation_state ∈ {definitive, disputed, insufficient_evidence}`. The "ambiguous → provisional + human gate" flow described in the legacy text below is **replaced** by the disputed terminal with `positions[]` populated (no human gate required for the routing decision; the disputed result is the user-facing answer per Owner Principle 4 — minimal owner review). Provisional record creation is permitted only after the match cell concludes "new scholar" per `REQ-SRC-0043.yaml` Phase 5 amendment, NOT during unresolved ambiguity.

> The legacy text below remains as historical context for non-source-engine call paths (excerpting-engine quoted-scholar resolution may still use the legacy thresholds during the Phase 5 implementation rollout). The legacy thresholds will be retired across all call paths during the Phase 5 implementation phase per the implementation handoff.

Each candidate is scored by the multi-signal matching algorithm (§4.A.2). Results fall into three ranges:
...
> **Phase 5 amendment 2026-04-30 (scholar-matching DR cross-provider synthesis):** The historical claim "this component does NOT use multi-model consensus" is **superseded**. The source-engine call path now uses ≥2 independent verifier consensus per `engines/source/spec/.../REQ-SRC-0052.yaml` (Stage-2 verifier cell with hybrid round-0/round-1 protocol) and compound 4-condition threshold per `REQ-SRC-0053.yaml` (definitive requires same chosen_id ∧ mean ≥ 0.92 ∧ each ≥ 0.90 ∧ no rival within 0.07 ∧ ≥2 non-name attributes per `INV-SRC-0013.yaml`). The ≥2-verifier consensus implements Critical Rule 7 (multi-model consensus for content decisions) and aligns with `D-041`. Stage-1 deterministic narrowing (per `REQ-SRC-0051.yaml`) is unchanged in its single-stream nature (deterministic operations are reproducible by design); consensus is applied at the LLM-reasoning Stage-2 layer where content judgment is required. The legacy single-LLM rationale below is preserved as historical context for the original (pre-Phase-5) design intent.

(legacy text — retained for historical reference; superseded by the Phase 5 amendment above)
...
| `SCHOLAR_DISAMBIGUATION` | Info | Matching score in ambiguous range (0.50–0.85) — **legacy path** for non-source-engine callers | Create provisional record + human gate checkpoint (legacy). Phase 5 amendment 2026-04-30: source-engine call path replaces this flow with `engines/source/spec/.../REQ-SRC-0053.yaml` disputed terminal (`CON-SRC-0008.disambiguation_state="disputed"` with `positions[]` populated) — no human gate required for the routing decision per Owner Principle 4 |
