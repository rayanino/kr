...
Notes: This was the lowest-confidence author in the entire corpus (Opus 0.55, CA 0.70). The low confidence is WELL-CALIBRATED — this is a modern author with limited general online presence (no Wikipedia article, no academic institution page), though he has multiple published works. The pipeline correctly identified him despite the low confidence. Authority_level disagreement: Opus=modern_compilation, CA not shown but result.json treats as standard. Opus's "modern_compilation" is a reasonable characterization of a 2022 1-page grammar summary.
