...
**Human gate decisions.** The owner's responses to placement escalations, evolution proposals, and coverage review requests. Each decision carries: `decision_id`, `gate_type` (one of `placement_review`, `evolution_review`, `coverage_review`), `decision` (one of `approve`, `reject`, `modify`), `modifications` (object or null), `reviewer_notes` (string or null), `timestamp`. **Idempotency rule:** if a decision arrives with a `decision_id` that has already been processed (checked against `library/sciences/{science_id}/gate_log.jsonl`), the duplicate is logged and ignored. This prevents double-application from network retries or UI resubmission.
...
(c) **Embedding similarity.** The engine computes a sentence embedding of `excerpt_topic` (using multilingual-e5-large or equivalent) and compares it against precomputed embeddings of all leaf titles in the science tree. The top 3 leaves by cosine similarity become additional candidates if they are not already in the candidate set. This catches terminology mismatches that LLM search might miss. Leaf title embeddings are computed at tree commitment (§4.A.3 Commitment phase) and recomputed after any evolution that adds, removes, or renames leaves. Embeddings are cached in `library/sciences/{science_id}/leaf_embeddings.npy` alongside a manifest mapping array indices to leaf paths. If the cache is missing or stale (manifest leaf count ≠ active tree leaf count), embeddings are recomputed before the first placement in that science.
...
**Pre-approval policies.** Per-source, per-science configuration records managed by the human gate engine. The human gate validates and stores these records after the owner approves 10+ consecutive placements from the same source in the same science without modification (i.e., 10 approvals in a row for that specific source-science pair with no `modify` or `reject` decisions between them; placements from other sources do not reset the count). Pre-approval can be revoked by the owner at any time. Pre-approval only affects the human gate step; all §4.A.2 validation checks still run.
...
**Excerpt redistribution.** Each excerpt in the proposal's redistribution plan is moved to its new leaf. The move is atomic: the placed excerpt file is written to the new leaf's directory, and the old file is removed, in a single transactional operation.
After each write, the engine performs the same post-write fidelity check as §5.1 (file exists, size > 0, `primary_text` byte-identical to original). If any redistribution fails (e.g., target directory creation fails, fidelity check fails), the entire evolution is rolled back.

Before any file operations begin, the engine writes a write-ahead log (WAL) to `library/sciences/{science_id}/evolution/wal_{proposal_id}.jsonl` recording every intended operation (source path, destination path, operation type).
The engine must check WAL write success (file exists, non-empty) and confirm it before proceeding with excerpt moves. The WAL is an operational artifact — it does not require content-level check beyond write success.
On failure, the WAL provides recovery instructions. On successful completion, the WAL is archived to the `applied/` directory alongside the proposal.
...
**Rollback.** If the owner requests rollback of an evolution, the taxonomy engine: (a) restores the previous tree version from `tree_history/`, (b) reverses all excerpt redistributions, (c) handles post-evolution excerpts — excerpts placed at the evolved leaves AFTER the evolution but BEFORE the rollback — by returning them to `draft` status and placing them in the re-placement queue (`library/sciences/{science_id}/placement_queue.jsonl`), (d) marks all affected entries as stale. Rollback is a human-gated operation.

**Rollback failure.** If a rollback fails midway (e.g., file system error during excerpt restoration), the engine enters `TAX_ROLLBACK_FAILURE` state — a fatal error that halts all taxonomy operations for the affected science.
The engine writes a diagnostic report to `library/sciences/{science_id}/evolution/rollback_failure_{timestamp}.json`.
The engine must check that this write succeeded (file exists) and confirm it before halting. This is an operational artifact — no content-level check required.
The report contains: the evolution being rolled back, which excerpts were successfully restored, which excerpts remain in the post-evolution state, and the file system error. The owner is notified immediately. Recovery requires manual intervention: an operator (Claude Code session) reads the diagnostic report and completes the rollback by moving the remaining excerpts. No automated retry is attempted because the file system may be in an inconsistent state.
...
**Placement validation.** After every placement decision, the engine verifies: (a) the confirmed leaf exists in the active tree, (b) the excerpt's `science_id` matches the tree's science, (c) the excerpt file was written successfully (file existence + size > 0), (d) the coverage analytics were updated, (e) the written file's `primary_text` field matches the input excerpt's `primary_text` byte-for-byte (a round-trip fidelity check that detects serialization corruption of Arabic text — see T-1 in reference/KNOWLEDGE_INTEGRITY.md). Failure of any check triggers `TAX_PLACEMENT_INTEGRITY_ERROR` (fatal — the placement is reverted and the excerpt returns to draft status).
...
**Cross-version placement validity.** After a tree evolution, a batch job checks that every excerpt in the science has a `confirmed_leaf` that resolves to a leaf in the current tree version. Excerpts referencing leaves that no longer exist (due to evolution that was not properly migrated) trigger `TAX_ORPHAN_DETECTED` (fatal — the excerpt is returned to `draft` status and placed in the re-placement queue).
...
### 5.4 Threat Prevention Mapping (reference/KNOWLEDGE_INTEGRITY.md)

Every threat from the knowledge integrity protocol is mapped below to taxonomy-specific vectors and prevention mechanisms.

**T-1 (Silent Text Corruption) → Taxonomy vector: serialization corruption during placement write.** The taxonomy engine writes the placed excerpt to disk as JSON. If the JSON serializer mishandles Arabic text (e.g., double-encoding UTF-8, stripping diacritics, normalizing Unicode forms), the `primary_text` in the placed file differs from the input. **Prevention:** (a) Post-write fidelity check in §5.1 — the engine re-reads the written file and compares `primary_text` byte-for-byte against the input. (b) The taxonomy engine never modifies `primary_text` — it is copied verbatim from the draft excerpt. (c) JSON serialization uses `ensure_ascii=False` to preserve Arabic characters without escape sequences. **Residual risk:** LOW — the post-write check catches any serialization corruption before the placement is committed.

**T-2 (Attribution Error) → Taxonomy vector: upstream school/author metadata propagated into analytics.** The taxonomy engine does not make attribution decisions, but it consumes `school`, `primary_author_id`, and `quoted_scholars` from upstream and uses them for coverage analytics (§4.A.6), disagreement topology (§4.B.4), and scholarly landscape (§4.B.6). If upstream attribution is wrong, all analytics are wrong. **Prevention:** (a) Cross-excerpt consistency check — if the same `primary_author_id` appears with different `school` values across excerpts, the inconsistency is logged as `TAX_METADATA_INCONSISTENCY` (warning) and a human gate checkpoint is created. (b) Disagreement topology marks leaves with only one source per school as `low_robustness` (§4.B.4), reducing confidence in thin evidence. (c) Scholarly landscape records all analytical claims with `excerpt_ids` evidence links — every assertion is traceable to the specific excerpts that support it. **Residual risk:** MEDIUM — the taxonomy engine cannot independently verify school attribution; it depends on the excerpting engine's consensus-based attribution (T-2 mitigation chain items 1–4 in reference/KNOWLEDGE_INTEGRITY.md).

**T-3 (Taxonomic Misplacement) → Taxonomy vector: this is the taxonomy engine's PRIMARY threat.** An excerpt placed at the wrong leaf means Rayane encounters it in the wrong context, which is a knowledge corruption. **Prevention:** (a) Two-stage placement with LLM + embedding candidates (§4.A.1). (b) Multi-model consensus for ambiguous cases scoring 0.5–0.8 (§6). (c) Human gate for all placements below 0.8 confidence or with ties (§5.3). (d) Pre-approval policies only after 10+ consecutive correct placements per source-science pair. (e) One-excerpt-per-source diagnostic (§4.A.2) detects when the same source sends 2+ excerpts to the same leaf, which may indicate a placement error or a too-coarse leaf. (f) Owner feedback loop — the owner can relocate misplaced excerpts at any time (§2.2, Scenario 8). **Residual risk:** LOW for individual excerpts (6 prevention layers make single-excerpt errors unlikely); MEDIUM for systematic bias (if the LLM consistently misclassifies a particular topic type, all excerpts of that type are wrong — see adversarial test §10.5.1).

**T-4 (Context Loss) → Taxonomy vector: placement separating an excerpt from its referenced context.** If an excerpt says "as mentioned above" and the "above" content is at a different leaf, the excerpt loses context. **Prevention:** This is primarily an excerpting engine responsibility (self-containment verification). The taxonomy engine's contribution: when placing an excerpt with `self_containment_score` < 0.6, the engine logs `TAX_LOW_SELF_CONTAINMENT` (info) as an additional signal. This does not block placement — self-containment is the excerpting engine's gate — but it creates a record that supports post-hoc quality analysis.

**T-5 (Synthesis Hallucination) → Taxonomy vector: scholarly landscape contains hallucinated relationships.** The landscape's influence chain (§4.B.6) and discourse transitions use LLM inference, which can hallucinate connections. **Prevention:** (a) Influence type 3 (temporal inference) is always marked `inferred: true` with explicit confidence scores — the synthesizer knows which relationships are inferred vs. evidenced. (b) Every landscape claim links to specific `excerpt_ids` — the synthesizer can verify claims. (c) Landscape confidence scoring (§4.B.6) penalizes thin evidence; landscapes with confidence < 0.4 are marked `preliminary`. (d) The taxonomy engine never claims certainty for inferred relationships — the output schema distinguishes `explicit_citation`, `teacher_student`, and `temporal_inference` influence types. **Residual risk:** MEDIUM for temporal inference; LOW for explicit citation and teacher-student (these are metadata-based, not LLM-inferred).

**T-6 (Metadata Poisoning) → Taxonomy vector: wrong `science_id` routes excerpt to wrong tree.** If the excerpting engine tags a Nahw excerpt as Sarf, the taxonomy engine places it in the Sarf tree. All downstream analytics (coverage, disagreement topology, landscape) are corrupted for both sciences. **Prevention:** (a) §2.1 validates that `science_id` matches a registered science. (b) If the taxonomy engine cannot place an excerpt (TAX_UNPLACEABLE, no candidate scores ≥ 0.5), one possible cause is a wrong `science_id` — the staging area notification to the owner includes this possibility: "This excerpt could not be placed in {science_name}. Possible causes: (1) the tree lacks the right leaf, or (2) the excerpt may belong to a different science." (c) Coverage analytics flag anomalies — if a source that is expected to contribute to 2+ sciences (based on the source's `science_scope` metadata) has all excerpts routed to a single science, this is suspicious. **Residual risk:** MEDIUM — the taxonomy engine cannot independently determine the correct science; it trusts upstream metadata. A wrong `science_id` with a plausible placement (score ≥ 0.5 in the wrong tree) would evade all detection.

**T-7 (Duplication and Contradiction) → Taxonomy vector: same content placed twice at same leaf from different sources.** **Prevention:** (a) Semantic deduplication detection (§4.A.8) flags excerpt pairs with cosine similarity > 0.85 at the same leaf. (b) Duplicate clusters are passed to the synthesizer, which presents the strongest source rather than repeating content. (c) Deduplication is detection-only — the taxonomy engine never merges or deletes excerpts, preventing accidental data loss. **Residual risk:** LOW — the 0.85 threshold is conservative; near-identical content is reliably detected. Paraphrased duplicates below the threshold may pass through, but this is a minor quality issue, not a knowledge corruption.

### 5.5 Error Cascade Analysis

This section traces how upstream errors and internal failures propagate through the taxonomy engine.

**Cascade 1: Wrong `science_id` from excerpting engine.**
- Excerpt enters wrong science tree → Stage 1b LLM searches wrong tree → may find a plausible leaf (cross-science terminology overlap is common in Islamic studies, e.g., "شروط" appears in every science) → excerpt placed at a leaf in the wrong science with moderate confidence → coverage metrics for BOTH the correct and incorrect science are wrong → disagreement topology may show false school positions → scholarly landscape builds false influence chains → synthesizer generates an entry with misattributed content.
- **Detection:** TAX_UNPLACEABLE (if no leaf scores ≥ 0.5, which only catches cases where the content clearly doesn't fit). For plausible cross-science placements, no automated detection exists. The owner's review (human gate for placements < 0.8) is the primary defense. For pre-approved sources, this cascade is undetectable until the owner reads the entry or coverage report.
- **Mitigation needed:** When a source's excerpts are first processed in a science, no pre-approval exists — all placements go through human gate. This provides the owner with visibility during the critical bootstrapping phase. After pre-approval, the risk of undetected science_id errors increases.

**Cascade 2: Corrupted tree YAML (valid structure, wrong semantics).**
- Tree loads successfully (valid YAML, valid hierarchy) but Arabic titles are wrong (e.g., "باب الطهارة" title on a leaf that should be "باب الصلاة") → all placement decisions based on the wrong titles → excerpts consistently placed at wrong leaves → coverage analytics reflect the wrong structure → entries generated for wrong topics.
- **Detection:** Tree integrity validation (§5.1) catches structural issues but NOT semantic issues. The validation phase of tree construction (§4.A.3) tests the tree against real content, which would catch semantic errors before commitment. Post-commitment, the only defense is the human gate (the owner reviews placement decisions and notices wrong leaf titles).
- **Mitigation:** The tree construction validation phase (§4.A.3) is a strong defense because it tests every leaf against real source content BEFORE the tree is committed. Once committed, semantic tree errors require owner detection.

**Cascade 3: Evolution redistribution with mid-process failure.**
- Evolution approved → tree YAML updated to new version → 5 of 8 excerpts redistributed → file system error on excerpt 6 → rollback triggered → tree YAML restored from history → 5 excerpts need reversal → rollback of excerpt 3 fails (original directory was already removed).
- **Detection:** `TAX_REDISTRIBUTION_FAILURE` is raised, triggering full rollback. If rollback also fails, `TAX_ROLLBACK_FAILURE` halts the science (§4.A.7 Rollback failure).
- **Mitigation:** Evolution uses a write-ahead log (WAL), an operational artifact that does not require content validation: before modifying any file, the engine records the intended operation (source path, destination path, operation type) in `library/sciences/{science_id}/evolution/wal_{proposal_id}.jsonl`. On failure, the WAL provides the recovery instructions. On successful completion, the WAL is archived. This makes both forward operations and rollback recoverable from any mid-point failure.

**Cascade 4: LLM consistently misclassifying a topic type.**
- A particular type of excerpt (e.g., methodological excerpts about أصول الفقه) consistently scores highest against a wrong leaf type (e.g., فروع/applied rulings) because the LLM conflates "evidence for a ruling" with "the ruling itself" → all excerpts of this type are systematically misplaced → coverage analytics show false density at the wrong leaf and false gaps at the correct leaf → disagreement topology is wrong for both leaves.
- **Detection:** Difficult. Individual placements score ≥ 0.8, so no human gate is triggered. The misplacement is systematic, not random. **Detection signals:** (a) coverage gap at the correct leaf despite sources that should populate it; (b) leaf capacity diagnostic fires at the wrong leaf (exceeding the 30-excerpt threshold); (c) one-excerpt-per-source diagnostic fires repeatedly at the wrong leaf.
- **Mitigation:** The gold baseline placement test (§10.1) is the primary defense. If the baseline includes examples of the tricky topic type, systematic misclassification is caught during testing. Post-deployment, the nightly coverage consistency check (§5.2) and the owner's coverage reviews (triggered after every 50 placements per science) are the secondary defenses.

**Cascade 5: Embedding model silently degraded.**
- Embedding model produces valid but low-quality vectors (e.g., after an update that breaks Arabic tokenization) → embedding candidates in Stage 1c are wrong → candidate set lacks the correct leaf → LLM in Stage 2 only sees wrong candidates → placement at best-of-bad-options.
- **Detection:** The embedding model failure (§7.2 `TAX_EMBEDDING_FAILURE`) only catches complete failures, not degradation. **Detection signal:** placement accuracy on the gold baseline drops — a regression test run detects the degradation.
- **Mitigation:** Monthly regression testing against the gold baseline (§10.3). If accuracy drops below the ≥ 85% threshold, an alert is generated. The engine can also monitor embedding consistency: if the cosine similarity between known-synonym leaf title pairs drops below 0.5, the embedding model is suspected degraded (`TAX_EMBEDDING_DEGRADED`, warning).
...
| `TAX_METADATA_INCONSISTENCY` | Warning | Same author appears with different school values across excerpts | Inconsistency logged. Human gate checkpoint created for resolution. |
| `TAX_LOW_SELF_CONTAINMENT` | Info | Excerpt placed with self_containment_score < 0.6 | Informational log for quality analysis. Does not block placement. |
| `TAX_EMBEDDING_DEGRADED` | Warning | Known-synonym leaf pairs have cosine similarity < 0.5 | Embedding model suspected degraded. Placement continues LLM-only until model is verified. Alert generated. |
...
| `TAX_ORPHAN_DETECTED` | Fatal | Post-evolution excerpt has invalid leaf | Excerpt returned to `draft` status and queued for re-placement. Alert generated. |
| `TAX_ROLLBACK_CONFLICT` | Warning | Rollback cannot place post-evolution excerpts | Post-evolution excerpts returned to `draft` status and queued for re-placement. |
| `TAX_ROLLBACK_FAILURE` | Fatal | Rollback itself fails midway | Science halted. Diagnostic report written. Manual recovery required (see §4.A.7 Rollback failure). |
...
### 10.5 Adversarial Test Cases

These tests simulate knowledge corruption scenarios. Each test must PASS (the engine prevents the corruption) or the engine has a vulnerability.

**§10.5.1 — Systematic Placement Bias.** Construct a set of 10 excerpts about أصول الفقه methodological principles (e.g., حجية خبر الآحاد, الإجماع, القياس) and submit them to the placement algorithm with `science_id: fiqh`. The correct placements are in usul-related leaves; the adversarial scenario is that the LLM conflates methodological discussions with their applied fiqh counterparts (e.g., placing a discussion of how to evaluate hadith evidence under a fiqh ruling that uses hadith evidence). **Pass criteria:** ≥ 8 of 10 excerpts placed at usul-adjacent leaves, not at applied fiqh leaves. If < 8 pass, the placement algorithm has a systematic bias that must be fixed before the affected science trees are populated. **Purpose:** Validates that the LLM distinguishes meta-discussion (about methodology) from object-level discussion (rulings themselves) — a distinction critical in Islamic studies but subtle for LLMs.

**§10.5.2 — Evolution Orphan Attack.** Create a leaf with 5 placed excerpts. Submit an evolution proposal that splits the leaf into 2 sub-leaves but whose redistribution plan only covers 4 of the 5 excerpts (one excerpt is "forgotten"). **Pass criteria:** The evolution invariant check (zero_orphans) detects the missing excerpt and the proposal is rejected with `TAX_EVOLUTION_INVARIANT_FAIL`. The 5th excerpt must not end up in limbo. **Purpose:** Validates that the zero-orphans invariant catches incomplete redistribution plans.

**§10.5.3 — Mid-Migration Crash Recovery.** Simulate an evolution with 6 excerpt redistributions. After writing the new tree YAML and redistributing 3 excerpts, inject a file system error (permission denied on the 4th excerpt's target directory). **Pass criteria:** (a) The engine triggers `TAX_REDISTRIBUTION_FAILURE`. (b) The WAL file exists and records all 6 intended operations. (c) After recovery, ALL 6 excerpts are at their pre-evolution leaves (complete rollback). (d) The tree YAML is restored to the previous version. (e) No excerpt has an inconsistent state (placed at a leaf that doesn't exist in the active tree). **Purpose:** Validates crash recovery and the WAL mechanism.

**§10.5.4 — Rollback with Post-Evolution Excerpts.** Apply an evolution that splits leaf A into A1 and A2. Place 3 new excerpts at A1 and 2 new excerpts at A2 (these are post-evolution excerpts). Then request rollback. **Pass criteria:** (a) The original excerpts that were redistributed from A are restored to A. (b) The 5 post-evolution excerpts are returned to `draft` status and placed in the re-placement queue (they cannot go to leaf A because A may not match their content — A was split for a reason). (c) Entries at A1 and A2 are marked stale. (d) No excerpt is lost. **Purpose:** Validates rollback completeness when new content arrived between evolution and rollback.

**§10.5.5 — Duplicate Human Gate Decision.** Submit a placement for human gate review. Send the approval decision twice (same `decision_id`). **Pass criteria:** The excerpt is placed exactly once. The second decision is logged as duplicate and ignored. The excerpt file is not written twice, the coverage metrics are not double-counted, and the gate log records both the original decision and the duplicate rejection. **Purpose:** Validates idempotency of human gate decision processing.

**§10.5.6 — Arabic Text Fidelity Through Placement.** Create a draft excerpt with `primary_text` containing: (a) text with full tashkeel (حَرَّمَ), (b) text with hamzat wasl and qat' (اِسْتَغْفَرَ vs أَسْتَغْفِرُ), (c) text mixing Arabic, parenthetical Latin transliteration, and Arabic numerals, (d) text with tatwil/kashida characters (ـ). Process the excerpt through placement and write it to disk.
Verify: the written file's `primary_text` is byte-identical to the input. No diacritics lost, no Unicode normalization applied, no characters substituted. **Pass criteria:** Post-write fidelity check (§5.1) passes for all four text variants. **Purpose:** Validates T-1 prevention through the placement write path — the most common source of silent Arabic text corruption.
