...
HR-30: **Every session that modifies engine code, SPEC, protocol, or scripts MUST dispatch both Codex CLI and Gemini CLI at least once.** Codex reviews structural/code quality. Gemini reviews scholarly accuracy. These are not optional — they are the two strongest code-level coworkers. CC subagents are a third cheat code: dispatch parallel agents for independent tasks. All three (Codex, Gemini, CC agents) must go through `/prompt-architect` (HR-23/HR-29). A session that writes code without Codex review or scholarly content without Gemini review has skipped its strongest quality check. Enforced by `stop-dispatch-gate.sh` (warns if no recent dispatch) and the handoff scorecard (must list all coworker dispatches).
