...
**FP-5 (Knowledge corruption is the worst failure):** One corrupted excerpt poisons the reader's trust in the entire library. Silent corruption is worse than visible failure. Always assume an excerpt is wrongfully excerpted until proven otherwise. The engine must fail loud — never silently produce an excerpt that looks correct but misrepresents the source. **Cascading trust collapse:** a wrong excerpt invalidates the entire book it came from (other excerpts from the same source cannot be trusted) → the excerpting engine itself becomes suspect → the owner's entire knowledge base requires re-evaluation. This cascade means the blast radius of one silent error is not one excerpt but the full corpus processed by the same prompt/model/configuration. **Two-tier corruption definition (DR adversarial review, session 2):** (1) *Confirmed corruption* — deterministic, source-faithfulness-breaking violations where `primary_text` cannot be proven to be a faithful extraction from the frozen source (text integrity check failure, offset mismatch producing wrong text, provenance chain broken). Action: hard stop + quarantine + invalidate downstream. (2) *Suspected corruption* — semantic risks where determinism cannot adjudicate (suspected decontextualization, unresolved cross-references, ambiguous attributions, heuristic mismatch). Action: flag + block owner consumption + continue processing with warnings. Without this two-tier definition, FP-5 becomes a denial-of-service trap: normal LLM diacritic variance would trigger pipeline-wide halt (owner F7, F8, 2026-04-04; Batch 1 hardening session 2; DR adversarial refinement).
