...
| M-13 | §4.B.6 / passaging §2 | `markers` and `confidence` vs `detection_confidence` | `LayerMapEntry` field name alignment | ✅ RESOLVED (commit e2d6043, Session 1). Renamed `detection_confidence` → `confidence`, added `markers: list[str] = []`. Passaging SPEC §2 updated for backward compatibility. |
| M-14 | §4.A.6 | 14 fields in SPEC example vs 7 fields in `DivisionNode` | `div_id`, `type`, `digestible`, `editor_inserted`, `page_hint_start`, `page_hint_end`, `child_div_ids` missing from model | ✅ RESOLVED (commit e2d6043, Session 1). Expanded `DivisionNode` to 9 fields (added `div_id` + `DivisionType` enum with 13 Arabic structural types). 5 redundant SPEC fields dropped per MUSTFIX_RESOLUTIONS.md. SPEC §4.A.6 example updated to match 9-field model. |
