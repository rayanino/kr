...
- heading_text: string or null. If non-null, the heading text is atomized as a heading structural type atom before the main text.
...
**Quran detection.** The engine uses a canonical Quran text database (all 6,236 ayat) and a fuzzy matching algorithm (based on the Quran_Detector approach: matching sequences of 3+ words against the canonical text). When a match is found with confidence ≥ 0.85, the span is marked as quran_ayah in embedded_refs with the surah and ayah numbers. The engine also detects common Quran introduction phrases ("قال تعالى"، "لقوله تعالى"، "قال الله"، "في قوله") that precede quotations. A Quran detection with confidence ≥ 0.95 (near-exact match against canonical text) is treated as a hard constraint — the LLM must classify this atom's scholarly function as evidence_quran unless the surrounding context makes clear the quotation is not being used as evidence (e.g., it appears in a tafsir as the text being explained, not as evidence for a claim).
...
**On validation failure.** The engine distinguishes between hard failures (V-2: offset integrity) and soft failures (V-6: suspicious layer distribution). Hard failures trigger re-atomization with the validation error included in the LLM prompt (up to 2 retries). Soft failures produce review flags on the affected atoms but do not block the atom stream from being written.
