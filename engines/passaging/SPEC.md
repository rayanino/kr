...
6. The division tree, if non-empty, has consistent `start_unit_index`/`end_unit_index` ranges that cover the content stream without overlap. Since `end_unit_index` is inclusive, the non-overlap constraint for sibling divisions A (preceding) and B (following) requires `B.start_unit_index > A.end_unit_index` (strictly greater). Inconsistency: `PSG_DIVISION_INCONSISTENT` (warning — fall back to flat passaging for affected regions).
...
**Continuity override audit.** When `boundary_continuity` overrides a character-level heuristic that would have produced a DIFFERENT join type (e.g., boundary_continuity says `mid_sentence` but the last character is sentence-terminal punctuation), log `PSG_ASSEMBLY_CONTINUITY_OVERRIDE` (info) with both signals recorded. This enables detection of systematic normalization engine bugs via log analysis.

**Single-unit passage text identity.** For single-unit passages, `passage_text` is identical to the content unit's `primary_text` — no trimming, no modification. For multi-unit passages, the assembled text begins with the first unit's `primary_text` (untrimmed) and ends with the last unit's `primary_text` (untrimmed). Joining rules (Rules 1–4) apply only at the BOUNDARIES between adjacent units.
...
**Text layer rebasing.** When assembling text from multiple content units, `text_layers` segments from each unit must be rebased to the assembled passage text. Before rebasing, the engine validates each segment: if any segment's `end` exceeds its content unit's `primary_text` length, the segment is clamped to the text length and `PSG_ASSEMBLY_LAYER_MISMATCH` (warning) is logged. Character offsets from per-unit segments are recalculated relative to the start of the assembled text. Adjacent segments from the same layer and author are merged into a single segment.
...
1. **Coverage check.** Two sub-checks: (a) Every content unit that is not flagged (`is_toc_page`, `is_index_page`, `is_blank` all false) must be covered by exactly one passage. `assert set(all_passage_unit_indices) == set(all_substantive_unit_indices)`. (b) **Gap correlation:** Compare the expected range `[0, total_content_units-1]` against the actual unit indices in the content stream. Every unit index in the expected range but missing from the stream must have been flagged by `PSG_CONTENT_GAP` during input validation (§2 check #5). If missing units exist but no gap was logged, this indicates a validation logic error: `PSG_VALIDATION_COVERAGE_GAP` (fatal).
...
**Technical approach.** When the normalization manifest includes a `content_census`, the passaging engine first validates the census values, then computes adapted parameters before processing begins:

**Census input validation.** Before using any census field, the engine validates: all density values (`technical_term_density`, `transition_density`, `mean_footnotes_per_page`) must be ≥0.0; all count values (`division_count`, `max_depth`) must be positive integers; `mean_pages_per_leaf_division` must be >0.0; `matn_ratio` must be in [0.0, 1.0]. If any value fails validation, the engine logs `PSG_ADAPTATION_FAILED` with reason "invalid census input: {field}={value}", skips all adaptation, and uses default configuration values.
...
**Discourse flow quality gate.** If discourse flow data is present but >80% of discourse segments in a division have type `unknown`, the engine treats the discourse flow as absent for that division and falls back to the keyword state machine. Log `PSG_DISCOURSE_FLOW_ABSENT` with reason "low quality (>80% unknown segments in division {div_id})."
...
- **Coverage difference >10%:** Use the signal whose argument spans cover a greater fraction of the text (higher coverage).
- **Coverage difference ≤10% AND argument count differs by >1:** Use the signal with MORE argument spans (more granular detection). Over-segmentation (too many arguments) is recoverable downstream; under-segmentation (too few arguments producing oversized passages) is not. Log with reason "granularity disagreement — preferring more specific signal."
- **Coverage difference ≤5% AND argument count differs by ≤1:** Use discourse flow (higher confidence because it was computed with per-page context).

Coverage is measured as the total character count of text within detected argument spans divided by the total text character count in the division. If the signals agree on argument boundaries (argument spans overlap ≥80% by character count), the engine uses discourse flow data and records `detection_source: "cross_validated"`.
...
**Nested argument handling.** A `فرع:` (subsidiary branch) inside a `مسألة:` increments `depth`. The sub-argument is tracked independently. The outer مسألة does not close until its own conclusion marker appears (at `depth == 1`) or a new top-level مسألة is encountered. Nesting depth is capped at 3 levels; at depth 4+, the engine treats additional openings as the "New argument opening at depth 1" transition in the state table — it implicitly closes all open nested arguments (resets `depth = 1`) and starts a new top-level argument. This cap prevents unbounded nesting from pathological input. When the cap fires, the engine logs `PSG_ARGUMENT_DEPTH_CAP_HIT` (info) and adds `review_flag: "argument_depth_exceeded"` to the affected passage.
...
| `definition` → `ruling` | 0.15 | Definition followed by its ruling — natural scholarly flow |
| `example` → `example` | 0.1 | Consecutive examples illustrating the same rule |
| `condition` → `condition` | 0.15 | Sequential listing of conditions for a ruling |
| `exception` → `exception` | 0.15 | Sequential listing of exceptions to a rule |
| `elaboration` → `elaboration` | 0.1 | Continued elaboration on the same point |
| `preferred` → `ruling` | 0.1 | Conclusion followed by formal ruling statement — natural close |
| `ruling` → `position` | 0.2 | Ruling stated, then scholarly positions presented |
| `narration` → `position` | 0.15 | Hadith narration followed by scholarly position analysis |
| `narration` → `narration` | 0.1 | Consecutive narrations on the same topic |
| `position` → `narration` | 0.3 | Scholarly position followed by supporting narration evidence |
| `exception` → `ruling` | 0.2 | Exception followed by ruling summary — natural flow |
| `example` → `ruling` | 0.2 | Example followed by its ruling — natural |
| `objection` → `objection` | 0.3 | Multiple objections in sequence before response |
| `condition` → `position` | 0.6 | Condition orphaned from its exception, then new topic |
| Any → mid-segment (no type transition) | 1.0 | Boundary falls INSIDE a discourse segment — worst case |

For `(from_type, to_type)` pairs not in this table, the default cost is 0.4. The `evidence_*` notation in the table is shorthand for all five evidence types in the discourse taxonomy: `evidence_quran`, `evidence_hadith`, `evidence_athar`, `evidence_qiyas`, `evidence_ijma`. The same cost applies to all of them in the default table. The 16-type discourse taxonomy is: `definition`, `ruling`, `position`, `evidence_quran`, `evidence_hadith`, `evidence_athar`, `evidence_qiyas`, `evidence_ijma`, `objection`, `response`, `preferred`, `example`, `condition`, `exception`, `elaboration`, `narration`. Per-science SCIENCE.md overrides (§8) may assign different costs to specific evidence types — for instance, hadith sciences may assign lower cost to `evidence_hadith → objection` (cost 0.6 instead of 0.8) because evidence-objection pairs are the primary scholarly unit in that genre.
...
1. If the fragment/partial passage can be merged with its successor without exceeding the hard maximum, merge them and re-forecast. This is a post-hoc boundary correction: the initial boundary placement (by size + division structure) produced a suboptimal split, and the completeness forecast caught it. Corrective merge is applied at most TWICE per initial fragment passage — merging at most 3 original passages into one. If after the first merge the passage is still a fragment/partial AND merging with the next successor would not exceed the hard maximum, a second merge is attempted. After two merges, no further merges are attempted regardless of forecast. This limits cascade growth while allowing reassembly of the common 3-part argument pattern (claim + evidence + conclusion). Log `PSG_COMPLETENESS_MERGE_REPAIR` (info) for each merge in the chain.
...
4. **Division granularity check.** If the average content units per leaf division is <1.5, the division tree is pathologically fine-grained (likely every page is its own division). Log `PSG_DIVISION_PATHOLOGICAL` (warning) and flag all passages from this source. This prevents the self-validation boundary integrity check (#8) from being trivially satisfied by division-boundary exceptions.
...
| `PSG_ASSEMBLY_FOOTNOTE_COLLISION` | Fatal | Footnote renumbering produced duplicate markers after sequential re-renumbering from 1 — indicates a code bug | Abort. Log with original markers. Sequential renumbering by order of appearance should never produce duplicates; a collision after renumbering indicates a logic error. |
...
| `PSG_ASSEMBLY_CONTINUITY_OVERRIDE` | Info | boundary_continuity overrode a character-level heuristic that would have produced a different join type | Log with both signals (continuity type vs. character heuristic type). No action needed — enables detection of systematic normalization bugs via log analysis. |
| `PSG_ARGUMENT_DEPTH_CAP_HIT` | Info | Argument nesting depth cap (3) reached; forced closure of nested arguments | Log with actual depth and affected division. Flag passage with `argument_depth_exceeded`. |
| `PSG_DIVISION_PATHOLOGICAL` | Warning | Average content units per leaf division <1.5 — division tree is pathologically fine-grained | Log. Flag all passages from this source. Consider re-normalization. |
| `PSG_VALIDATION_COVERAGE_GAP` | Fatal | Missing unit indices exist in expected range but no PSG_CONTENT_GAP was logged during input validation | Abort. Indicates a validation logic error — gap detection in §2 check #5 failed. |
