...
**Pre-human-gate default.** When `verified_flagged_status` is absent from a placed excerpt (because the human gate step and source trust tiers are not yet implemented), the synthesis engine treats the excerpt as `"verified"`. This is the safe default: all excerpts enter the factual layer. When the human gate step is implemented upstream, this default will be removed and the field will become truly mandatory. See taxonomy `CORE_EXTRACTION.md` for the deferral record.

**File format.** Each placed excerpt is a single JSON file at `library/sciences/{science_id}/content/{leaf_path}/excerpts/{excerpt_id}.json`. The file contains all upstream excerpt fields (D-023 preserved) merged with taxonomy placement additions (`lifecycle_stage`, `placement_route`, `confirmed_leaf`, `placement_confidence`, `placed_utc`, `taxonomy_version_at_placement`, `placement_reasoning`, `primary_topic_used`, `review_metadata`, `tie_detected`). Encoding: UTF-8, `ensure_ascii=False`. The synthesis engine reads all `.json` files in the leaf's `excerpts/` directory.

**Expected fields — degraded synthesis on absence.** `primary_author_id` (if absent, the entry cannot attribute the position to a specific scholar — the engine proceeds but marks the attribution as "unknown author"), `school` (if absent for a science that has schools, the entry notes the gap), `school_confidence` (if absent, school assignment is treated as fully confident; if present and < 0.5 for a science that has schools, the excerpt is treated as unknown-school and enters the inference path in §4.A.2 rather than being directly partitioned — this prevents low-confidence school assignments from contaminating per-school entries), `content_types` (if absent, the engine cannot tag the excerpt's role), `evidence_refs` (if absent, the entry cannot describe the evidence base), `self_containment_score` (if absent, all excerpts treated as equally self-contained), `placement_confidence` (if absent, all excerpts treated as equally well-placed), `processing_metadata` (if absent, no confidence-aware synthesis is possible).
  ...
  scholarly_positions: [
    {
      ...
      consensus_strength: string or null,  // §4.B.1: "absolute_consensus" | "near_consensus" | "strong_majority" | "weak_majority" | "evenly_disputed" | "minority_aberrant"
      confidence: float
    }
  ],
...
1. Source metadata via `source_id` → `library/sources/{source_id}/metadata.json`. Extracts: work title (`title_arabic`), genre, authority level (`authority_level`), trust tier, text fidelity, publisher, muhaqiq. The source registry (`library/registries/sources.json`) provides `trust_tier` and `processing_status` for quick filtering but does not carry bibliographic fields — the full metadata record is the authoritative chain source (D-023).
2. Scholar authority via `primary_author_id` → `library/registries/scholars.json`. Extracts: full name, death dates (hijri + CE), school affiliations, teachers, students, scholarly standing, sectarian tradition, methodological stance.
...
6. Scholarly context via source metadata → `scholarly_context` field. When non-null, extracts: `composition_date_hijri`, `composition_period`, `tradition_position`, `known_textual_issues`, `historical_significance`, `muhaqiq_reputation`, `tahqiq_methodology_note`, `edition_known_issues`, `context_richness`, `uncertain_dimensions`. When `scholarly_context` is null (inference was skipped or failed), the engine proceeds with registry-only data — the analytical layer loses narrative enrichment but the factual layer is unaffected. When present, `context_richness` determines reliance level: `"rich"` → full use in analytical layer; `"partial"` → selective use; `"minimal"` → treat as absent. Before using any specific field, the engine checks `uncertain_dimensions` — if the field name appears there, it is not used for generated claims.
...
**Unattributed excerpt escalation.** After school partitioning is complete, the engine checks whether any unattributed excerpt (excluded from all per-school entries due to unknown school) represents a unique position. For each unattributed excerpt, the engine computes semantic similarity between the excerpt's `primary_text` and every position summary identified from attributed excerpts. If no position summary has cosine similarity ≥ 0.7 with the unattributed excerpt, the engine escalates from warning to human gate checkpoint: "An excerpt from an unidentified scholar may represent a unique position not captured in any school-group entry." This prevents silent loss of unique positions due to metadata gaps.

**Duplicate cluster verification.** Before accepting a duplicate cluster from the taxonomy engine's coverage analytics, the engine verifies that all excerpts in the cluster support the same scholarly position. For each cluster with ≥ 2 excerpts, the engine sends all cluster members to the `entailment_model` (cross-provider, §8) with the prompt: "Do these excerpts express the same scholarly position, or do they reach different conclusions?" If any excerpt disagrees with the cluster's apparent stance, the cluster is broken and all excerpts are treated individually. This prevents incorrect upstream deduplication from silently merging distinct positions.
...
**Step 4: Intra-author contradiction detection.** For each author with ≥ 2 excerpts at the leaf, the engine compares positions. Implementation: the LLM receives all excerpts from the same author and returns a `ContradictionCheckResult`: `author_id`, `contradictions_found` (boolean), `contradictions` (array of `{excerpt_id_a, excerpt_id_b, nature}` where `nature` is one of: `retraction` — later work supersedes earlier based on composition dates, `general_vs_specific` — one is a general rule, the other addresses a specific case, `genuine` — unexplained contradiction, `context_dependent` — both are correct in different contexts). Retraction detection uses a three-level fallback for chronological ordering of an author's works: (1) `scholarly_context.composition_date_hijri` from each excerpt's source metadata — when both works have this field, direct comparison determines which is later; (2) `scholarly_context.composition_period` — when the structured date is unavailable but the narrative classification is present (e.g., "early work" vs "late comprehensive synthesis"), the LLM interprets the relative ordering; (3) author death dates as a rough proxy — since all works share the same author and thus the same death date, this provides zero ordering information for same-author comparisons and is only useful when combined with other signals. Works from the same author where all three levels return null are flagged as `undatable`.
...
**Step 4: Entailment verification.** For each generated sentence, the engine verifies that the sentence is entailed by its attributed excerpt spans. This MUST use a model from a different provider than the model used for sentence generation in Step 3, regardless of whether consensus is triggered for Phase 2. This cross-provider requirement prevents self-reinforcing hallucination: when generator and verifier share the same parametric knowledge (same model family), the verifier may confirm a hallucination that both models "know" to be plausible. The verifier receives the generated sentence and the attributed spans and returns: `entailed` (boolean), `unsupported_elements` (list of specific sub-claims not supported by the spans). Sentences that fail entailment verification are rewritten (up to 2 retries) with the unsupported elements removed or moved to the analytical layer. If all retries fail, the sentence is marked with a `weak_grounding` flag and included with a note.

**Vacuous entailment defense.** A sentence that is trivially true (e.g., "scholars discussed this topic") is technically entailed by any excerpt but conveys no information. After entailment passes, the engine computes semantic similarity between the generated sentence and its attributed spans (using the Arabic embedding model). If similarity is below 0.3, the sentence is flagged as potentially vacuous — it passed entailment but shares no substantive content with the source. Vacuous sentences are rewritten with a prompt that explicitly requires incorporating specific content from the attributed spans. If rewriting fails (2 retries), the planned claim is dropped from the factual layer and logged in `generation_metadata`.
...
**Source scholarly context integration.** When available (`scholarly_context` is non-null with `context_richness` of `"rich"` or `"partial"`), the engine weaves source-level context into the analytical narrative. All scholarly context claims carry `grounding_type: "library_metadata"` — they were inferred during source intake, not during synthesis.

- `tradition_position` informs authority weighting: a work described as "the standard matn in the Shafi'i nahw curriculum for 700 years" carries more narrative weight than one described as "marginal work, rarely cited." The engine may note: "This definition comes from [Work], [tradition_position]."
- `historical_significance` provides framing for the work's role in the scholarly lineage. When non-null, the engine incorporates it into the temporal narrative alongside `composition_period`. E.g., "This work, [historical_significance], represents [composition_period] of the author's output."
- `known_textual_issues` and `edition_known_issues` surface as quality caveats in the analytical layer when they affect excerpts used in the entry. E.g., if a source has `known_textual_issues: ["Popular editions omit the final chapter on al-waqf"]` and excerpts from the final chapter are used, the engine notes: "This excerpt comes from a portion of the work that some editions omit — the cited edition [muhaqiq/publisher] includes it." Edition-specific issues (`edition_known_issues`) are noted in citation commentary: "Note: this edition is known to have [issue]."
- `muhaqiq_reputation` and `tahqiq_methodology_note` contextualize edition quality in the analytical layer. When a citation references a source with known tahqiq strengths, the entry may note: "cited from the [muhaqiq] edition, [tahqiq_methodology_note]." These signals complement the trust evaluation's `tahqiq_quality` factor — trust is a score; these notes explain why.

The engine checks `uncertain_dimensions` before building narrative claims on any specific scholarly_context field. Fields listed in `uncertain_dimensions` are not presented as established facts — they may inform the LLM's own research prompts but are not cited as `"library_metadata"` claims.

**Scholarly context absence.** When `scholarly_context` is null for all sources at a leaf, the analytical layer relies entirely on (1) cross-excerpt analysis, (2) scholar authority registry data (death dates, school affiliations, teachers/students), and (3) LLM research. The entry quality is not degraded for the factual layer — only the analytical layer's narrative depth is reduced.
...
**Check 3: School isolation.** For per-school entries: no excerpt from another school in `source_excerpt_ids`. Violation: critical failure. Additionally, the engine validates that no excerpt's author has a `sectarian_tradition` value (from the scholar authority record) that conflicts with the entry's assumed tradition. An entry for Sunni fiqh must not include excerpts from Twelver Shi'i scholars without explicit annotation. This catches a class of errors that school-level isolation misses — two scholars may both be tagged "Hanbali" but if metadata was incorrectly inferred, one could actually belong to a different sectarian tradition. A `sectarian_tradition` mismatch is a critical failure with the same retry logic as school isolation.
...
**T-5 (Synthesis Hallucination) → Synthesis vector: the LLM invents scholarly positions, evidence, or attributions not in any excerpt.** This is the defining threat for the synthesizing engine. **Prevention:** (a) Attribution-first pipeline (§4.A.4.1) — content selection before generation, not post-hoc citation. (b) Cross-provider entailment verification (§4.A.4.1 Step 4) — every factual sentence checked against its attributed spans by a model from a different provider than the generator, preventing self-reinforcing hallucination. (c) Vacuous entailment defense (§4.A.4.1) — sentences that pass entailment but share no substantive content with their spans are caught and rewritten. (d) Grounding type enforcement (§4.A.5 Check 6) — no `"llm_research"` claims in `core_treatment` or `scholarly_positions`. (e) Socratic self-verification (§4.B.6) — questions generated from the entry are answered from the entry text; inconsistencies reveal hallucinated content. (f) The structural separation between factual and analytical layers ensures that even when LLM knowledge is used, it is confined to sections the reader knows are not excerpt-backed. **Residual risk:** MEDIUM — subtle hallucinations (e.g., correct scholar, correct school, plausible but wrong death date) may pass all checks. Multi-model consensus for high-significance leaves is the primary mitigation for these cases.
...
| `entailment_model` | `gpt-4o` | Any supported LLM (different provider from primary_model) | Model for entailment verification (§4.A.4.1 Step 4). MUST be a different provider than `primary_model` to prevent self-reinforcing hallucination |
...
### 10.4 Adversarial Test Scenarios (Hardening-Verified)

These scenarios were designed during HARDENING to stress-test the threat mitigations in §5.4 and the attribution-first pipeline in §4.A.4.1. Each must be implemented as a test case.

#### 10.4.1 Attribution-First Pipeline Adversarial Scenarios

**ADV-ATTR-1: All excerpts have low self-containment.** Input: 5 excerpts at a leaf, all with `self_containment_score` < 0.3 (e.g., each says "والحكم كما تقدم" or "وتمامه في ما سبق"). Expected: (a) Phase 1 flags all excerpts as low self-containment in `generation_metadata`, (b) Step 2 attribution succeeds but spans are short/uninformative, (c) Step 3 conditioned generation cannot make substantive claims from these spans, (d) Step 4 entailment passes only for vacuous sentences, (e) the vacuous entailment defense (§4.A.4.1) catches these and drops them from the factual layer, (f) if all claims are dropped, `SYNTH_NO_GROUNDED_CLAIMS` triggers a diagnostic entry (§4.A.8). Verify: the engine never produces a plausible-sounding factual entry from context-free excerpts.

**ADV-ATTR-2: Excerpt metadata contradicts primary_text.** Input: An excerpt with `primary_author_id` pointing to Ibn Qudamah (Hanbali, d. 620 AH), but `primary_text` begins "قال الشافعي رحمه الله" (al-Shafi'i said). This is a multi-layer text where the excerpt captures a quotation. Expected: (a) Phase 2 position identification reads the text and may identify al-Shafi'i's position, (b) the structured `holders` array uses `primary_author_id` (Ibn Qudamah) as the citing scholar and recognizes al-Shafi'i as a `quoted_scholars` reference, (c) the entry says "Ibn Qudamah quotes al-Shafi'i as saying..." not "al-Shafi'i said...", (d) if `quoted_scholars` metadata is absent, the engine detects the discrepancy between metadata-author and text-author and logs `SYNTH_METADATA_RESOLUTION_FAILED`. Verify: the entry never misattributes a quoted position to the quoting author or vice versa.

**ADV-ATTR-3: Self-reinforcing hallucination attempt.** Input: 3 Basran nahw excerpts about المبتدأ. The LLM's parametric knowledge includes a well-known Kufan position not present in any excerpt. Expected: (a) Step 1 outline may include a PlannedClaim for the Kufan position, (b) Step 2 finds no supporting excerpt → claim routed to analytical layer with `grounding_type: "llm_research"`, (c) Step 3 never generates this claim in the factual layer, (d) Check 6 (grounding type consistency) catches any leakage. The cross-provider entailment verification (§4.A.4.1 Step 4) ensures that even if the generator subtly embeds the Kufan position within a Basran claim, a model from a different provider family is less likely to confirm the hallucination. Verify: (a) no Kufan position appears in `core_treatment` or `scholarly_positions`, (b) if it appears in `analytical_layer`, it is clearly marked as `grounding_type: "llm_research"`.

#### 10.4.2 Per-Threat Adversarial Scenarios

**ADV-T1: Single diacritic corruption.** Input: A matn excerpt where the frozen source has "حَرَّمَ" (harrama — made forbidden) but the normalized text was corrupted to "حَرَمَ" (harama — was forbidden/sacred). This reverses the meaning. Expected: (a) the engine quotes the corrupted text verbatim per the direct quotation policy (§4.A.4.1), (b) the citation includes `source_id` + `passage_id` + page reference enabling the owner to spot-check, (c) if the corrupted definition contradicts edge cases at the same leaf, §4.B.6 self-verification catches the incoherence. Residual risk verified: the engine CANNOT independently detect this corruption. Test passes if the entry is traceable (the owner can find and fix the error), not if the corruption is prevented.

**ADV-T2: Multi-layer misattribution.** Input: An excerpt from شرح ابن عقيل (Ibn Aqil's commentary on al-Alfiyyah). The excerpt contains: (a) a line from al-Alfiyyah itself (by Ibn Malik, d. 672 AH), (b) Ibn Aqil's commentary (d. 769 AH). The `primary_author_id` is correctly Ibn Aqil, but the excerpt's `content_types` does not distinguish matn from sharh layers. Expected: (a) Phase 2 position identification recognizes both scholars' contributions, (b) the entry attributes the verse to Ibn Malik and the commentary to Ibn Aqil, (c) if the engine cannot distinguish layers (no `content_types` layer marking), it attributes the entire excerpt to Ibn Aqil with a note that the excerpt contains quoted material. Verify: the entry never attributes Ibn Malik's verse to Ibn Aqil.

**ADV-T3: Closely related topic misplacement.** Input: An excerpt about "conditions of وضوء" (wudu) placed under "conditions of صلاة" (prayer). Both topics share vocabulary (طهارة, شرط). Expected: (a) Phase 2 tahrir al-mas'ala (§4.A.3 Step 3) detects the excerpt answers a different question than the other excerpts, (b) the engine emits `TAX_EVOLUTION_SIGNAL` with type `leaf_split_suggested`, (c) the entry either excludes the misplaced excerpt or clearly flags it as potentially off-topic. Verify: a misplaced excerpt on a closely related topic is at least flagged, even if not caught with certainty.

**ADV-T4: Forward-reference excerpt.** Input: An excerpt whose `primary_text` is "والحكم في هذا كالحكم في المسألة المتقدمة مع زيادة شرط الرضا" (The ruling here is like the ruling in the previous issue, with the addition of requiring consent). `self_containment_score` = 0.25. Expected: (a) Phase 1 flags low self-containment, (b) Step 2 can only attribute "requiring consent" — the "previous issue" is not in the excerpt, (c) Step 3 generates only what is attributable: the consent requirement, (d) Step 4 entailment catches any attempt to specify what "the previous issue" was, (e) the entry notes that this excerpt references prior context not available in the library. Verify: the entry never fabricates what "the previous issue" was.

**ADV-T5: Plausible but wrong death date.** Input: Scholar authority record correctly shows al-Nawawi d. 676 AH. The LLM's parametric knowledge incorrectly "knows" he died in 677 AH. Expected: (a) the engine reads death dates from the scholar authority registry, not from LLM knowledge (§5.4 T-2 prevention (b)), (b) Check 7 (temporal consistency) catches any date in the generated text that doesn't match the registry, (c) the entry shows 676 AH. Verify: registry data always overrides LLM knowledge for structured metadata.

**ADV-T6: Misclassified source type.** Input: A compilation (مصدر ثانوي — secondary source) incorrectly tagged as primary (مصدر أصلي) in the source registry. Expected: (a) Phase 1 deduplication gives it undue priority as representative of its cluster, (b) the library composition bias assessment (§4.A.3 Step 7) detects if this source dominates (high HHI), (c) the entry's analytical layer notes the source concentration. Residual risk verified: the engine cannot independently verify source classification. Test passes if source concentration is detected and disclosed.

**ADV-T7: Near-identical editions.** Input: Two editions of المغني by ابن قدامة, both placed at the same leaf. Edition A has minor textual differences from Edition B (different tahqiq). Expected: (a) Phase 1 semantic deduplication (threshold 0.92) detects them as duplicates, (b) representative selection picks the higher-fidelity edition, (c) the citation notes "also found in [other edition]", (d) the entry does not present two identical positions. Verify: when the similarity is 0.90 (below threshold), the engine creates two separate positions from the same author — caught by Step 4 (intra-author contradiction detection) as `context_dependent` or `general_vs_specific`.

#### 10.4.3 Error Cascade Analysis

**Cascade 1: Metadata resolution failure → lost position.** Chain: (a) `library/registries/scholars.json` is missing the entry for scholar X, (b) §4.A.2 metadata resolution logs `SYNTH_METADATA_RESOLUTION_FAILED` for scholar X's excerpt, (c) the excerpt's `school` field is null (it depended on the scholar registry for school inference), (d) §4.A.2 school inference fails (no authority record), (e) the excerpt enters the unattributed pool, (f) the excerpt is excluded from all per-school entries, (g) a scholarly position is lost. **Impact:** An entire position disappears from the entry with only a warning-level log. **Mitigation verification:** The `generation_metadata` logs unattributed excerpts. The quality assessment (§4.B.4) penalizes entries that exclude excerpts. The owner sees degraded quality scores and can investigate. **Residual gap found:** There is no explicit alert when an unattributed excerpt represents a UNIQUE position not held by any attributed scholar. **Fix applied:** Added to §4.A.2: when an unattributed excerpt's `primary_text` does not semantically match any identified position from attributed excerpts (cosine similarity < 0.7 with all position summaries), the engine escalates from warning to human gate checkpoint with message: "An excerpt from an unidentified scholar may represent a unique position not captured in any school-group entry."

**Cascade 2: Wrong duplicate cluster → merged distinct positions.** Chain: (a) The taxonomy engine's semantic deduplication (threshold 0.92) incorrectly clusters two genuinely different excerpts (they share similar vocabulary but reach opposite conclusions), (b) §4.A.2 selects a representative from the cluster, (c) the non-representative excerpt is silently excluded from individual citation, (d) its distinct position is never identified in Phase 2, (e) the entry is missing a position. **Impact:** A scholarly position is silently lost — worse than Cascade 1 because no warning is generated. **Mitigation verification:** The intra-author contradiction detection (§4.A.3 Step 4) would catch this IF both excerpts are from the same author. If from different authors, the loss is invisible. **Residual gap found:** No defense against incorrect upstream duplicate clustering for different-author excerpts. **Fix applied:** Added to §4.A.2: before accepting a duplicate cluster, the engine verifies that all excerpts in the cluster support the same position (a lightweight LLM check comparing the position stance of each excerpt). If any excerpt in the cluster disagrees with the cluster's representative, the cluster is broken and all excerpts are treated individually. This check uses the `entailment_model` (cross-provider) to avoid the same bias that created the cluster.

### 10.5 Contract Verification (Hardening-Verified)

Field-by-field verification between synthesis §2 input contract and taxonomy §3 output contract.

**Verified matches (no issues):**
- `excerpt_id`: Taxonomy guarantees uniqueness per §3.1. Synthesis requires it. ✓
- `source_id`: Taxonomy D-023 pass-through. Synthesis requires it. ✓
- `confirmed_leaf`: Taxonomy adds it. Synthesis requires it. ✓
- `lifecycle_stage`: Taxonomy sets to `placed`. Synthesis requires `placed`. ✓
- `verified_flagged_status`: Taxonomy adds it. Synthesis requires it. ✓
- `primary_author_id`, `school`, `content_types`, `evidence_refs`, `self_containment_score`, `placement_confidence`, `processing_metadata`: All in taxonomy D-023 or added fields. All in synthesis expected fields. ✓

**Issues found and resolved:**
- `primary_text`: Not explicitly listed in taxonomy §3.1 D-023 pass-through (it lists metadata fields, not core content). Resolution: `primary_text` is part of the base excerpt schema from the excerpting engine; the taxonomy engine preserves all fields ("All upstream metadata is preserved by reference"). No action needed — the field is preserved, just not explicitly enumerated because D-023 addresses metadata, and `primary_text` is content. Documented here for clarity.
- `school_confidence`: Taxonomy passes it through (D-023) but synthesis §2.1 did not list it. **Fixed** in this session — now listed as an expected field with handling rules.
- Taxonomy-added fields not consumed by synthesis: `placed_utc`, `review_metadata`, `taxonomy_version_at_placement`, `placement_reasoning`. These are available but the synthesis engine has no rules that reference them. `taxonomy_version_at_placement` could support debugging but is not needed for entry generation. No action required — these fields flow through without being consumed, which is acceptable per D-023.
