...
- `genre` must be one of the `Genre` enum values in contracts.py (20 values: `matn`, `sharh`, `hashiyah`, `mukhtasar`, `nazm`, `risalah`, `rihlah`, `taqrirat`, `mawsuah`, `fatawa`, `mujam`, `tabaqat`, `fiqh_comparative`, `hadith_collection`, `tafsir`, `sirah`, `tarikh`, `adab`, `usul_al_fiqh`, `other`)
- Title keywords are strong signals: "شرح" → `sharh`, "مختصر" → `mukhtasar`, "حاشية" → `hashiyah`, "نظم" → `nazm`, "رسالة" → `risalah`, "رحلة" → `rihlah`
- No additional genres may be added without updating both the SPEC and the `Genre` enum
- The LLM prompt and `library/config/genre_synonyms.json` must include all Genre enum values and their Arabic synonyms.
...
5. **Consistency cross-check.** Inferred fields are checked for mutual consistency:
   ...
   - **hashiyah + is_multi_layer=False with no layers** triggers human gate (severity="gate"), not just warning, because hashiyah structurally requires 3 layers (matn→sharh→hashiyah) — this is always a contradiction. sharh + no layers remains a warning (could be a classification boundary issue near sharh/risalah).
   - Inconsistencies are flagged as warnings (not blocking) and trigger `needs_review` on the inconsistent fields, EXCEPT author-science mismatch and hashiyah+no-layers which trigger human gates.
   - **Single-model death date warning (5g).** When only one consensus model provides a `death_date_hijri` and the other returns null, AND the death date source is `"inference"` (not extracted from the book's metadata card), flag `SRC_DEATH_DATE_UNVERIFIED` (warning) and add `death_date_hijri` to `needs_review_fields`. This catches the ERR-03 hallucination pattern where one model fabricates a specific date while the other correctly abstains.
...
This comparison runs alongside the author identification consensus (same LLM calls, no additional cost). It addresses reference/KNOWLEDGE_INTEGRITY.md T-2 mitigation #1 ("Multi-model consensus for **all** attribution decisions"), which requires that the certainty of an attribution — not just the identity — is verified. No downstream engine checks `attribution_status`; this is the only verification point.
...
| `SRC_DEATH_DATE_UNVERIFIED` | Warning | Single-model inferred death date (ERR-03 pattern) | Add `death_date_hijri` to `needs_review_fields`. Date preserved but flagged for owner verification. |
