...
| INV-SRC-0015 | invariant | Scholar-match provenance completeness on every emitted result | confirmed | critical |
...
### INV-SRC-0015 — Scholar-match provenance completeness on every emitted result
- Type: invariant
- Layer: quality
- Step: n/a
- Status: confirmed
- Priority: critical
- Confidence: high
- Source: Phase 5 scholar-matching DR synthesis (2026-04-30) §5.3 (renumbered from ChatGPT DR's stale INV-SRC-0012 to INV-SRC-0015 per repo verification — slot 0012 already taken by "Non-applicable genres require level null"). Aligns with D-023 metadata-pass-through invariant (Critical Rule 6) and Critical Rule 3 (every claim is traceable). Codex Stage-3 Defect 2 fix applied: registry_release_version is the canonical name (snapshot_version is FORBIDDEN). Cross- provider 2-of-2 ratification on provenance discipline: Claude DR §3d + ChatGPT DR §3d both require structured provenance on every match-call result.
- Rule: Every CON-SRC-0008 ScholarMatchResult emitted by the scholar match cell MUST satisfy four completeness predicates: (1) evidence_sources is non-empty when disambiguation_state ∈ {definitive, disputed}; (2) provenance.registry_release_version is non-null and matches the case-pinned snapshot from REQ-SRC-0049 (the canonical field name; snapshot_version is FORBIDDEN per Codex Stage-3 Defect 2); (3) provenance.threshold_audit records all 4 compound- threshold predicates from REQ-SRC-0053 (mean_passes [bool], both_pass [bool], no_rival_close [bool], corroboration_count_ge_2 [bool]) along with their numeric backing values (mean_confidence [float], leader_confidence [float], rival_confidence [float], corroboration_count [int]); (4) provenance.stage_2 _verifier_record contains verifier_a_id, verifier_b_id, prompt_template_hashes, seeds, and round_count for EVERY result regardless of disambiguation_state. Insufficient_evidence terminals MAY have empty evidence_sources (no positive identification surfaced) but MUST still satisfy predicates (2), (3), and (4) so the audit trail records WHY the case fell to insufficient_evidence (which compound-threshold predicate failed; what verifier setup ran).
...
- Postconditions:
  ...
  - Connector-token preservation in nasab_chain: each nasab_chain entry preserves the input connector form byte-faithfully — a fragment containing "بن X" emits nasab_chain=["بن X"]; a fragment containing "ابن X" emits nasab_chain=["ابن X"]. The parser does NOT canonicalize between بن (chain-internal short form) and ابن (standalone long form) because the connector distinction can carry attribution information for some classical naming conventions. Downstream comparison against registry records performs connector-equivalence matching at the comparison layer, not at the parsed_components storage layer.
...
- Acceptance criteria:
  ...
  - AC-2 [deterministic] Given Fragment "عبد الله بن المبارك" (compound name + nasab, short connector form بن).; When REQ-SRC-0050 normalization runs; Then parsed_components.ism = "عبد الله" (compound preserved as single semantic unit, NOT split into "عبد" and "الله"); parsed_components.nasab_chain = ["بن المبارك"] (connector form preserved byte-faithfully per the connector-preservation postcondition; same rule as AC-3 where input بن also surfaces as بن in the chain). The compound rule is enforced. No SRC-E-COMPOUND-NAME-SPLIT error..
